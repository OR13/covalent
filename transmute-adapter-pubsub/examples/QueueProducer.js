...
var QueueProducer = function(solaceModule, queueName) {
  ...
  producer.connect = function(argv) {
    ...
    // create session
    try {
      producer.session = solace.SolclientFactory.createSession(config);
    } catch (error) {
      producer.log(error.toString());
    }
    // define session event listeners
    producer.session.on(solace.SessionEventCode.UP_NOTICE, function(
      sessionEvent
    ) {
      producer.log(
        "=== Successfully connected and ready to send messages. ==="
      );
      producer.sendMessage();
      producer.exit();
    });
    producer.session.on(solace.SessionEventCode.CONNECT_FAILED_ERROR, function(
      sessionEvent
    ) {
      producer.log(
    ...
    producer.session.on(solace.SessionEventCode.DISCONNECTED, function(
      sessionEvent
    ) {
  ...
  producer.sendMessage = function() {
    if (producer.session !== null) {
      ...
      producer.log(
        "Cannot send messages because not connected to Solace message router."
      );
    }
  };
